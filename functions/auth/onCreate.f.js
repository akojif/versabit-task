...
export default functions.auth.user().onCreate((uRecord, context) => {
    ...
    return admin
        ...
        .then(user => {
            ...
            const provider =
                user.providerData !== [] ?
                user.providerData[0] : { providerId: email ? 'password' : 'phone' }
            ...
            if (email) {
                const mailOptions = {
                    from: `"Akoji Francis" <${gmailEmail}>`,
                    to: email,
                    subject: `Welcome to Versabit Task!`,
                    text: `Hey ${displayName ||
            ''}!, Welcome to Versabit Task. I hope you will enjoy the demo application.`,
                }
